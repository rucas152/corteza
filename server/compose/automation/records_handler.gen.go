...
type (
	...
	recordsSearchResults struct {
		...
		NextPage       string
		PrevPage       string
		PageNavigation []expr.TypedValue
	}
)
...
func (h recordsHandler) Search() *atypes.Function {
	return &atypes.Function{
		...
		Handler: func(ctx context.Context, in *expr.Vars) (out *expr.Vars, err error) {
			...
			{
				// converting results.NextPage (string) to String
				var (
					tval expr.TypedValue
				)

				if tval, err = h.reg.Type("String").Cast(results.NextPage); err != nil {
					return
				} else if err = expr.Assign(out, "nextPage", tval); err != nil {
					return
				}
			}

			{
				// converting results.PrevPage (string) to String
				var (
					tval expr.TypedValue
				)

				if tval, err = h.reg.Type("String").Cast(results.PrevPage); err != nil {
					return
				} else if err = expr.Assign(out, "prevPage", tval); err != nil {
					return
				}
			}

			{
				// converting results.PageNavigation ([]expr.TypedValue) to Array
				var (
					tval expr.TypedValue
				)

				if tval, err = h.reg.Type("Array").Cast(results.PageNavigation); err != nil {
					return
				} else if err = expr.Assign(out, "pageNavigation", tval); err != nil {
					return
				}
			}
...
func (a recordsDeleteArgs) GetModule() (bool, uint64, string, *types.Module) {
	return a.hasModule, a.moduleID, a.moduleHandle, a.moduleRes
}

func (a recordsDeleteArgs) GetNamespace() (bool, uint64, string, *types.Namespace) {
	return a.hasNamespace, a.namespaceID, a.namespaceHandle, a.namespaceRes
}
